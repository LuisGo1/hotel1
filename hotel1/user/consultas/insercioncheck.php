...
try {
    ...
    $insertCheckInOutQuery = "INSERT INTO check_in_out (fecha_check_in, id_cliente, estado, id_habitacion, cant_dias, id_empleado)
                              VALUES (?, ?, 'ocupada', ?, ?, ?)";
    ...
    $insertCheckInOutStmt->bind_param("siisi", $fechaCheckIn, $clienteId, $cuartoId, $cantDias, $empleadoId);

    if (!$insertCheckInOutStmt->execute()) {
        throw new Exception("Error al insertar en check_in_out.");
    }

    $updateHabitacionQuery = "UPDATE habitaciones SET estado = 'ocupado' WHERE cuarto_id = ?";
